...
@component
class DocumentEmbeddingExtractor:
    @component.output_types(reference_embeddings=List[List[float]])
    def run(self, documents: List[Document]) -> Dict[str, Any]:
        embeddings = [doc.embedding for doc in documents if doc.embedding is not None]
        return {"reference_embeddings": embeddings}
...
@component
class SimilarityNNComponent:
    def __init__(self, model):
        """Initialize with a trained SimilarityNN model."""
        self.model = model

    @component.output_types(similarities=List[float], top_k_embeddings=List[List[float]])
    def run(self, query_embedding: List[float], reference_embeddings: List[List[float]]) -> Dict[str, Any]:
        """Compute similarity scores for a query against reference embeddings."""
        similarities = []
        query_tensor = torch.tensor(query_embedding).float()  # Convert to tensor
        
        for ref_emb in reference_embeddings:
            ref_tensor = torch.tensor(ref_emb).float()
            similarity_score = self.model(query_tensor, ref_tensor).item()  # Forward pass
            similarities.append(similarity_score)

        # Sort by similarity score (descending) and get top-10 results
        top_10_indices = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)[:10]

        return {
            "similarities": [similarities[i] for i in top_10_indices],  # Top-10 similarity scores
            "top_k_embeddings": [reference_embeddings[i] for i in top_10_indices]  # Corresponding embeddings
        }
...
class Conversation:
    ...
    def _create_pipeline(self, api_key: str, document_store: InMemoryDocumentStore) -> Pipeline:
        ...
        embedding_extractor = DocumentEmbeddingExtractor()
        model = SimilarityNN(embedding_dim=768)
        similarity_component = SimilarityNNComponent(model=model)
        ...
        pipeline.add_component("similarity_calculator", instance=similarity_component)
        pipeline.add_component("retriever", QdrantEmbeddingRetriever(document_store=document_store, top_k=25))
        pipeline.add_component("embedding_extractor", instance=embedding_extractor)
        ...
        pipeline.connect("retriever.documents", "embedding_extractor.documents")
        pipeline.connect("embedding_extractor.reference_embeddings", "similarity_calculator.reference_embeddings")  # Use embeddings here
        pipeline.connect("text_embedder.embedding", "similarity_calculator.query_embedding")
        pipeline.connect("similarity_calculator.top_k_embeddings", "prompt_builder.documents")
        pipeline.connect("prompt_builder.prompt", "llm.messages")

        # SimilarityNN connections (to calculate similarity scores)
    #    pipeline.connect("text_embedder.embedding", "similarity_calculator.query_embedding")
    #   pipeline.connect("similarity_calculator.similarities", "prompt_builder.documents")
    ...
    def send_message(self, message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "rag_documents": result["retriever"]['documents'],
...
def chat_loop(conversation: Conversation):
   while True:
       ...
       print(f"☀️ Retrieved Documents: {result['rag_documents'][:5]}")
